...
class CampaignOrchestrator:
    ...
    async def run_campaign(self, campaign_id: int) -> None:
        ...
        if not accounts:
            logger.error("Campaign %d has no accounts", campaign_id)
            return

        status = campaign["status"]
        logger.info("Starting campaign %d '%s' (status: %s)", campaign_id, campaign["name"], status)
        ...
        try:
            ...
            if status in (CREATED, WARMING, SCALING, ACTIVE):
            ...
            logger.error("Campaign %d error: %s", campaign_id, e)
            await self.db.update_campaign(campaign_id, status=PAUSED)

    async def _warmup_phase(self, campaign_id: int, accounts: list[str]) -> None:
        """Run warmup for all accounts in parallel."""
        logger.info("Starting warmup phase for campaign %d", campaign_id)
        ...
        if all_warmed:
            logger.info("All accounts warmed up for campaign %d", campaign_id)

    async def _sending_phase(self, campaign_id: int, accounts: list[str],
                              campaign: dict) -> None:
        """Run the DM sending phase with gradual scaling."""
        logger.info("Starting sending phase for campaign %d", campaign_id)
        ...
        while True:
            ...
            if camp and camp["status"] == PAUSED:
                logger.info("Campaign %d is paused", campaign_id)
                return
            ...
            if not targets:
                logger.info("No more pending targets for campaign %d", campaign_id)
                break
            ...
            logger.info("Day %d complete, waiting for next active period", day_counter)
    ...
    async def resume_campaign(self, campaign_id: int) -> None:
        ...
        if campaign["status"] != PAUSED:
            logger.warning("Campaign %d is not paused (status: %s)", campaign_id, campaign["status"])
            return
        await self.db.update_campaign(campaign_id, status=ACTIVE)
